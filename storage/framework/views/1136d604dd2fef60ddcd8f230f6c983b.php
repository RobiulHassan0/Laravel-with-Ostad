                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            <label class="form-label" for="stockAdjustQuantity">Quantity <span
                                    class="text-danger">*</span></label>
                            <input type="number" name="quantity" id="stockAdjustQuantity" class="form-control" min="1"
...
<?php $__env->startPush('scripts'); ?>
    <script>
        async function doStockAdjustment(){
            ...
            let note = document.getElementById('stockAdjustNote').value.trim();
            ...
            let url = '<?php echo e(url("/api/v1/stocks/stockAdjustment")); ?>';
            let token = localStorage.getItem('token');
            saveBtn.disabled = true;

            try{
                let response = await axios.post(url, objectt, {headers: {Authorization: 'Bearer ' + token }});

                if(response.data && response.data.success){
                    showSuccessToast(response.data.message || 'Stock Adjustment created successfully.');
                    let modalElem = document.getElementById('stockAdjustModal');
...
<?php $__env->stopPush(); ?><?php /**PATH C:\xampp\htdocs\Full Stack Laravel Projects by Ostad\inventory_project\resources\views/admin/stock/adjust.blade.php ENDPATH**/ ?>