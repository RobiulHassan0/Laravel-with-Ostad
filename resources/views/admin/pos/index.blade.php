            ...
            <div class="card cart-sticky">
                ...
                <div class="card-body p-0">
                    ...
                    <div class="border-top p-3 bg-light" style="display: none" id="totalSelection">
                        ...
                        <div class="d-flex justify-content-between mb-2 text-danger" style="display: none"
                            id="itemDiscountRow">
    ...
    <script>
        ...
        function renderProducts() {
            ...
            filtered.forEach(function (product) {
                ...
                if (!isOutOfStock) {
                    let card = colDiv.querySelector('.pos-product-card');
                    if (card) {
        ...
        function updateItemDiscount(productId, discountType, discountValue) {
            ...
            item.discount_value = Math.max(0, parseFloat(discountValue) || 0);
        ...
        function recalcCart() {
            cart.forEach(function (item) {
                ...
                if (item.discount_type === 'fixed') {
                    ...
                    let percentage = Math.min(item.discount_value, 100);
                    discountAmount = lineBeforeDiscount * (percentage / 100);
                }

                item.discount_amount = Math.round(discountAmount * 100) / 100;
                item.line_total = Math.max(0, Math.round((lineBeforeDiscount - item.discount_amount) * 100) / 100);
            })
        ...
        function getInvoiceDiscountAmount() {
            ...
            if (type === 'percent') {
                let percentage = Math.min(value, 100)
        ...
        function updateTotals() {
            ...
            let grandTotal = Math.max(0, Math.round((subtotal - invoiceDiscountAmount) * 100) / 100);
        ...
        function buildInvoicePayload(status) {
            ...
            let grandTotal = Math.max(0, Math.round((subtotal - discountAmount) * 100) / 100);
        ...
        document.getElementById('invoiceDiscountType').addEventListener('change', function () { recalcCart(); });
        document.getElementById('invoiceDiscountValue').addEventListener('input', function () { recalcCart(); });
