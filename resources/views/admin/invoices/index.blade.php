        ...
        <script>
            ...
            async function getInvoices() {
                ...
                try {
                    ...
                    invoiceData = response.data['invoice_list'] || [];
                    tbody.innerHTML = '';
                    if (invoiceData.length === 0) {
                        tbody.innerHTML = `<tr><td colspan="9" class="text-center text-muted py-4">No invoice found</td></tr>`;
                        return;
                    }
                    invoiceData.forEach((item) => {
                        let invoiceDate = item['invoice_date'] ? item['invoice_date'].substring(0, 10) : '-';
                        let itemsCount = item['invoice_items'] ? item['invoice_items'].length : 0;
                        ...
                        let actionsHtml = `<button type="button" class="btn btn-sm btn-outline-primary" onclick="viewInvoice(${item['id']})" title="view">
                                    <i class="bi bi-eye"></i>
                                    </button>`;
                        ...
                        if (status === 'draft') {
                            ...
                            actionsHtml += `<button type="button" class="btn btn-sm btn-outline-danger" disabled title="Can not Delete finalized">
                                    <i class="bi bi-trash"></i>
                                    </button>`;
                        } else {
                            actionsHtml += `<button type="button" class="btn btn-sm btn-outline-danger" disabled>
                                    <i class="bi bi-trash"></i>
                                    </button>`;
                        }
                    ...
                    showErrorMessage(getErrorMessage(err, 'Failed to load invoices.'));
                }
            }
